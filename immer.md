...
Async producers
允许从返回Promise对象。 或者，换句话说，使用async / await。 这对于长时间运行的进程非常有用，只有在promise链解析resolve后才生成新对象。 请注意，如果生成器是异步的，则produce本身（即使是以curried形式）将返回promise。 例
